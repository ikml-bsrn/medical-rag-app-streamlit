...
@router.post("/api/v1", response_model=QueryResponse)
async def query_rag_system_post(request: Request, body: QueryRequest):
    ...
    try:
        full_response = ""
        async for chunk in rag_chain.astream(body.query):
            full_response += chunk
        
        return QueryResponse(query=body.query, response=full_response)
